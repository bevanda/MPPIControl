function [S] = cost_function(p, p_dot, theta, theta_dot, u, param)
    ...
    S = (6*(p)^2 + 12*(1+cos(theta))^2 + 0.1*u^2 + 0.1*theta_dot^2 + 0.1*p_dot^2)*dt;
...
%     S = (1*(p)^2 + 500*(1+cos(theta))^2 + 1*theta_dot^2 + 1*p_dot^2)*dt;
    ...
    % Cost function given in paper Information Theoretic MPC
    % for Model-Based Reinforcement learning
%      S = (10*(p)^2 + 500*(1+cos(theta))^2 + 1*theta_dot^2 + 15*p_dot^2)*dt;
